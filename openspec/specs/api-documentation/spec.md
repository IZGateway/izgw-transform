...
This capability defines how the IZ Gateway Transformation Service exposes interactive API documentation (Swagger UI) and the OpenAPI specification for its `/api/v1/**` REST surface, including how Swagger UI static resources are resolved across `org.webjars:swagger-ui` version changes and how access to these endpoints is controlled.
