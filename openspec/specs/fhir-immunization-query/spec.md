# fhir-immunization-query Specification

## Purpose

This capability defines how the IZ Gateway Transformation Service handles FHIR immunization query requests, including a non-standard convenience alias that lets senders supply a `subject` search parameter in place of `patient` on the FHIR query endpoints. The alias is applied by rewriting the request input before the `v2tofhir` mapping, so the outbound HL7 v2 QBP_Q11 query and the downstream Hub/IIS message contracts remain unchanged.
