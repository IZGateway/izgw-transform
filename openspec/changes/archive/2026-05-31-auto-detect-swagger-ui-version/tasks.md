...
- [x] 1.1 Add a new class `SwaggerUiVersionConfig` under `src/main/java/gov/cdc/izgateway/xform/configuration/` annotated with `@Configuration` and `@Slf4j`. (Implementation note: the original task spec'd a constructor-injected `@PostConstruct` design; that was abandoned during implementation because `spring.main.lazy-initialization: true` prevents the bean from being instantiated. See `design.md` Decision 2.)
- [x] 1.2 Expose a `static @Bean BeanPostProcessor` from `SwaggerUiVersionConfig` whose `postProcessAfterInitialization` matches `SwaggerUiConfigProperties`, calls `new org.webjars.WebJarVersionLocator().version("swagger-ui")`, and when the returned value is non-blank, calls `setVersion(detected)` on the bean. The detector is wrapped in a `Supplier<String>` exposed via a package-private `alignVersion(props, detector)` overload so failure-path tests can drive null / blank / throwing without mocking.
...
- [x] 1.4 On null/blank result or thrown `RuntimeException`, catch and emit one `WARN` log line including the cause and leave the version field untouched — satisfies the "Detection failure logs a warning" scenario; the application MUST still start. Failure paths are exercised by `SwaggerUiVersionConfigTests` via the `Supplier`-based overload.
...
- [x] 2.1 Add a JUnit 5 test class `SwaggerUiVersionConfigTests` under `src/test/java/gov/cdc/izgateway/xform/configuration/` that exercises the aligner directly against a fresh `SwaggerUiConfigProperties` instance and asserts the resulting version equals `WebJarVersionLocator.version("swagger-ui")` and matches `X.Y.Z`. Plain unit tests rather than `@SpringBootTest`; the real-container assertion lives in `SwaggerUiVersionContextTests`.
- [x] 2.2 In the same test class, add a test that asserts the resolved value is NOT the legacy hardcoded `5.32.5` (regression guard) and is NOT Springdoc's bundled default `5.32.2` when the BOM-resolved webjar version differs. **Extended** with three failure-path tests (null / blank / throwing detector preserves the existing version and does not propagate exceptions) and one pre-existing-pin-override test, all via the `Supplier`-based overload.
- [~] 2.3 ~~Add a `@SpringBootTest` integration test that issues `GET /swagger/ui.html`~~ — **Dropped.** No existing mTLS HTTP test plumbing in `src/test/java/`; standing it up would be disproportionate to the ~30-line production change. End-to-end Swagger UI is covered by manual task 3.4 against the local fat-jar. Replaced with `SwaggerUiVersionContextTests` (task 2.7) that asserts the BPP fires in the real Spring context — catches wiring regressions without the HTTP plumbing.
...
- [x] 2.7 Add `SwaggerUiVersionContextTests` (`@SpringBootTest`) that injects `SwaggerUiConfigProperties` and asserts its `version` equals `WebJarVersionLocator.version("swagger-ui")` after the real container boots — guards against future bean-matching regressions (e.g., a Springdoc rename or conditional removal) that the unit tests can't catch.
