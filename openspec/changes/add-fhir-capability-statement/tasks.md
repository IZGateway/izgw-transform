...
- [x] 1.2a Populate the remaining R4-required elements: `date` (fixed constant for determinism), `kind = instance`, and — because kind `instance` requires it (invariants cpb-2/cpb-14) — `implementation.description = "IZ Gateway Transformation Service"`. Added post-review with a `metadataSatisfiesR4RequiredElements` test.
